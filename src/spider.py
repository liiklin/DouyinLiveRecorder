...
# App-side (reflow) request headers. The default cookie is an anonymous ttwid:
# the app endpoints answer "it triggered risk control" when no cookie is sent.
DOUYIN_APP_HEADERS = {
...
def _douyin_app_headers(cookies: OptionalStr = None) -> dict:
    headers = dict(DOUYIN_APP_HEADERS)
    if cookies:
        headers['Cookie'] = cookies
    return headers
...
async def get_douyin_app_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    headers = _douyin_app_headers(cookies)
...
def _merge_douyin_origin_urls(room_data: dict) -> None:
    """Merge the original (原画) pull URLs into the room's pull maps in place.

    Both the web and the app payload embed an "origin" entry inside
    stream_url.live_core_sdk_data; the plain pull maps only carry the transcoded
    qualities, so the original one has to be merged back by hand.
    """
    ...
    stream_data = ''
    if pull_datas:
        stream_data = (pull_datas[list(pull_datas.keys())[0]] or {}).get('stream_data', '')
    if not stream_data:
        stream_data = (live_core_sdk_data.get('pull_data') or {}).get('stream_data', '')
    if not stream_data:
        return
    try:
        ...
        return
    origin = (json_data.get('data') or {}).get('origin') or {}
    origin_main = origin.get('main') or {}
    if not origin_main.get('hls') and not origin_main.get('flv'):
        return
    try:
        sdk_params = json.loads(origin_main.get('sdk_params') or '{}')
    except (TypeError, ValueError):
        sdk_params = {}
    codec = sdk_params.get('VCodec') or ''
    origin_m3u8 = {'ORIGIN': str(origin_main.get('hls') or '') + '&codec=' + codec}
    origin_flv = {'ORIGIN': str(origin_main.get('flv') or '') + '&codec=' + codec}
    stream_url['hls_pull_url_map'] = {**origin_m3u8, **(stream_url.get('hls_pull_url_map') or {})}
    stream_url['flv_pull_url'] = {**origin_flv, **(stream_url.get('flv_pull_url') or {})}


@trace_error_decorator
async def get_douyin_app_stream_data_by_room_id(room_id: str, proxy_addr: OptionalStr = None,
                                                cookies: OptionalStr = None) -> dict:
    """Resolve a Douyin room from its 19-digit room_id through the app endpoint.

    Follow-page live links (www.douyin.com/follow/live/<room_id>) and app share
    links embed the 19-digit room_id, while the web enter endpoint only accepts
    the 12-digit web_rid and answers status_code 4001038 for a room_id. The app
    reflow endpoint accepts room_id on its own and returns the same
    stream_url/hls_pull_url_map/flv_pull_url shape as the web path.
    """
    headers = _douyin_app_headers(cookies)
    ...
    try:
        ...
        if not room_data:
            raise Exception(f"{room_id} room data is empty")
        room_data['anchor_name'] = (room_data.get('owner') or {}).get('nickname', '')
        _merge_douyin_origin_urls(room_data)
    except Exception as e:
        print(f"Error message: {e} Error line: {e.__traceback__.tb_lineno}")
        room_data = {'anchor_name': ""}
    return room_data
